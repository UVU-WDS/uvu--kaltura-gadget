...
class KalturaEventNotificationTemplateService extends KalturaServiceBase
{
	 ...
	 * @param array $userParamsArray User params
	 * @return KalturaPushNotificationData
	 */
	function register($notificationTemplateSystemName, array $userParamsArray)
	{
		$kparams = array();
		$this->client->addParam($kparams, "notificationTemplateSystemName", $notificationTemplateSystemName);
		foreach($userParamsArray as $index => $obj)
		{
			$this->client->addParam($kparams, "userParamsArray:$index", $obj->toParams());
		}
